  ...
  return regl({
    vert: glsl`
      ...
      float computeBoxSize(vec3 boxCenter) {
        float noise = noise4d(
          vec4(boxCenter * NOISE_SCALE, time)
        );
        float unitNoise = (noise + 1.0) * 0.5;
        return mix(0.0, 1.0, unitNoise);
      }

      void main() {
        vec3 boxCenter2 = boxCenter;
        vec3 modifiedPosition =
          // Move the position to be centered about the origin.
          (position - boxCenter2)
          // Modify the scale.
          * computeBoxSize(boxCenter2)
          // Move it back to its original location.
          + boxCenter2;

        modifiedPosition.y +=
          sin(boxCenter2.x * 30.0 + time * 5.0) * 0.01 +
          sin(boxCenter2.z * 30.0 + time * 5.0) * 0.01;
